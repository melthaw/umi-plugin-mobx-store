...
export default (api: IApi): void => {
    ...
    api.onGenerateFiles(() => {
        ...
        models.forEach((path) => {
            ...
            const upperCasedImportDefaultSpecifier = importDefaultSpecifier[0].toUpperCase() + importDefaultSpecifier.slice(1);

            imports.push(`import { ${upperCasedImportDefaultSpecifier} } from '${path}';`);
            declareFields.push(`private ${importDefaultSpecifier}: ${upperCasedImportDefaultSpecifier};`)
            initFields.push(`this.${importDefaultSpecifier} = new ${upperCasedImportDefaultSpecifier}(this);`);
        });
        api.writeTmpFile({
            path: `${DIR}/root.ts`,
            content: Mustache.render(TPL_ROOT, {
                imports: imports.join('\n'),
                declareFields: declareFields.join('\n'),
                initFields: initFields.join('\n'),
            }),
        });
        api.writeTmpFile({
            path: `${DIR}/index.ts`,
